...
完整的无量纲化之前的质量分数格式表达为y(r) = （一区mg的质量流率*颗粒表面mg的质量分数-一区mg的之质量流率）*exp（（一区总的质量流率）/（4*pi*rho*D）*（（1/r_p）-（1/r））+一区mg的质量流率。
...
0824 
反应热问题   初始火焰温度猜测高点 可以计算


0825 
计算有结果 但部分粒径计算不收敛  整体偏小  尝试更改初始值猜测 调小


0826
debug经验    颗粒表面温度系数 *2 方程无问题   
颗粒表面能量方程约束可以适当放宽 结果 *0.1 
远场系数 *5
火焰面能量严格约束 *2
初值猜测中的火焰温度 可调  （出现 exitag 3  但是部分方程残差仍较大，可调火焰温度猜测应该可以控制方程6 或者方程7降到）